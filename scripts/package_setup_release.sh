...
#
# Writes: dist/fear-effect-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix fear-effect \
  --exe-name Fear_Effect_Recompiled \
  --display-name "Fear Effect Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --runtime-dir mods \
  --disc-hint "Fear Effect (USA) (Disc 1).cue" \
